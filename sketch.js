...
const SCALE_SPEED_RANGE = [0.25, 0.7];       // rad/frame gốc
const SPEED_FACTOR      = 1 / 12;            // chậm

// Biến cho layer
let overlayPG;
let holes = []; // mỗi hole = {x,y,w,h,r,amp,speed,phase}
...
function setup() {
  ...
  noFill();
  stroke(255, 150);
  const cnv = createCanvas(1920, 1080); // kích thước khớp với CSS #p5-holder
...
function draw() {
  ...
  for (let c of cells) {
    updateCell(c);
    const t = c.isActive ? frameCount * (NOISE_SPEED * 4) : tBase;
    drawCell(c, t);
  }
}
...
function windowResized() {
  ...
  overlayPG.pixelDensity(1);
  buildHoles(); // random lại cho nhanh, (nếu muốn giữ, cần lưu % theo width/height)
}
...
function drawGradientBackground(pg, paletteArr, darkHex, fadeT, alphaVal) {
  ...
  for (let y = 0; y < pg.height; y++) {
    ...
    let cFinal = lerpColor(cGrad, dark, fadeT); // pha dần sang tối
...
/*************************  PHÍM TẮT  *************************/
function keyPressed() {
  if (key === 'o' || key === 'O') buildHoles(); // random lại lỗ
}

const SCROLL_SPEED = 1.2; // chỉnh tốc độ ngang

function mouseWheel(e) {
  // delta dương khi lăn xuống
  const dx = e.delta * SCROLL_SPEED;
  window.scrollTo({
    left: window.scrollX + dx,
    top: 0,                 // luôn giữ top = 0
    behavior: 'auto'
  });

  return false; // chặn default (scroll dọc)
}